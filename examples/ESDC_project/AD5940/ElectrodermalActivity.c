...
AD5940Err AppEDACtrl(int32_t EDACtrl, void *pPara)
{
  switch (EDACtrl)
  {
    case APPCTRL_START:
    {
      ...
      if(AD5940_WakeUp(20) > 20)  /* Wakeup AFE by read register, read 10 times at most */
...
static AD5940Err AppEDASeqCfgGen(void)
{
  ...
  // dsp_cfg.StatCfg.StatSample = 
  dsp_cfg.DftCfg.HanWinEn = AppEDACfg.HanWinEn;
  AD5940_DSPCfgS(&dsp_cfg);
  AD5940_ADCRepeatCfgS(5*(4+2)+1);  /* (n+2)*osr + 1, n=4,osr=5*/
  // AD5940_ADCRepeatCfgS(1);
  // AD5940_ADCRepeatCfgS(5*(8+2));  /* (n+2)*osr + 1, n=4,osr=5*/
...
static AD5940Err AppEDASeqMeasureGen(void)
{
  ...
  AD5940_SEQGenCtrl(bFALSE); /* Stop sequencer generator */
  error = AD5940_SEQGenFetchSeq(&pSeqCmd, &SeqLen);
  // printf("SeqLen %d; AppEDACfg.InitSeqInfo.SeqLen %d\r\n", SeqLen, AppEDACfg.InitSeqInfo.SeqLen);
  if(error == AD5940ERR_OK)
  {
    if (AppEDACfg.MaxSeqLen < (SeqLen + AppEDACfg.InitSeqInfo.SeqLen)) return AD5940ERR_BUFF; /* Buffer limited */
...
static AD5940Err AppEDARtiaCal(void)
{
  ...
  if(AppEDACfg.RtiaAutoScaleEnable == bTRUE)
  {
    int i = AppEDACfg.RtiaAutoScaleMin;
	  // printf("RtiaAutoScaleMin: %d; RtiaAutoScaleMax: %d\r\n",i, AppEDACfg.RtiaAutoScaleMax);
...
AD5940Err AppEDAInit(uint32_t *pBuffer, uint32_t BufferSize)
{
  ...
  // printf("AD5940_SEQCfg AD5940_ADIID:%04x\r\n", AD5940_ReadReg(REG_AFECON_ADIID));
  /* Do RTIA calibration */
  ...
  // printf("AppEDARtiaCal AD5940_ADIID:%04x\r\n", AD5940_ReadReg(REG_AFECON_ADIID));
  /* Reconfigure FIFO */
  ...
  // printf("AD5940_FIFOCfg AD5940_ADIID:%04x\r\n", AD5940_ReadReg(REG_AFECON_ADIID));
  ...
  // printf("AD5940_SEQCtrlS AD5940_ADIID:%04x\r\n", AD5940_ReadReg(REG_AFECON_ADIID));
  ...
  // printf("AD5940_AFEPwrBW AD5940_ADIID:%04x\r\n", AD5940_ReadReg(REG_AFECON_ADIID));
...
static AD5940Err AppEDADataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  *pDataCount = 0;
  ...
  {
	  printf("Error in AppEDADataProcess\r\n");
	  while(1);
  }
