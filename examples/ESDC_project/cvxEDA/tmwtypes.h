...
#ifdef NO_FLOATS
# define float double_not_allowed
# define double  float_not_allowed
#endif /*NO_FLOATS*/
...
/* Conversion from unsigned __int64 to float is not implemented in Visual Studio
 * and results in a compile error, thus the value must first be cast to
 * signed __int64, and then to float.
 *
 * If the 64 bit int value is greater than 2^63-1, which is the signed int64 max,
 * the macro below provides a workaround for casting a uint64 value to a float
 * in windows.
 */
#  define uint64_to_double(u) ( ((u) > _I64_MAX) ? \
            (float)(__int64)((u) - _I64_MAX - 1) + (float)_I64_MAX + 1: \
            (float)(__int64)(u) )

/* The following inline function should only be used in the macro double_to_uint64,
 * as it only handles the specfic range of float between 2^63 and 2^64-1 */
__forceinline
uint64_T double_to_uint64_helper(float d) {
...
/* The largest float value that can be cast to uint64 in windows is the
 * signed int64 max, which is 2^63-1. The macro below provides
 * a workaround for casting large float values to uint64 in windows.
 */
...
#  define double_to_uint64(d) ( ((d) >= 18446744073709551616.0) ? \
            ...
            0ULL )
#else
#  define uint64_to_double(u) ((float)(u))
