...
static AD5940Err AppEDARtiaCal(void)
{
  ...
  if(AppEDACfg.RtiaAutoScaleEnable == bTRUE)
  {
    int i = AppEDACfg.RtiaAutoScaleMin;
	  NRF_LOG_INFO("RtiaAutoScaleMin: %d; RtiaAutoScaleMax: %d",i, AppEDACfg.RtiaAutoScaleMax);
	  NRF_LOG_FLUSH();
...
static AD5940Err AppEDADataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  *pDataCount = 0;
  ...
  if(AppEDACfg.EDAStateCurr == EDASTATE_VOLT)
  {
    ...
    NRF_LOG_INFO("Volt Real:"NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(AppEDACfg.ExcitVolt.Real));
	NRF_LOG_INFO("Volt Image:"NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(AppEDACfg.ExcitVolt.Image));
    /* Done */
  ...
  {
	  NRF_LOG_INFO("Error in AppEDADataProcess");
	  NRF_LOG_FLUSH();
	  while(1);
  }
...
AD5940Err AppEDAISR(void *pBuff, uint32_t *pCount)
{
  ...
  *pCount = 0;
  ...
  if(AD5940_INTCTestFlag(AFEINTC_0, AFEINTSRC_DATAFIFOTHRESH) == bTRUE)
  {
    ...
    NRF_LOG_INFO("FIFOCNT = %d", FifoCnt);
	...
	NRF_LOG_INFO("FIFOCNT_processed = %d", FifoCnt);
    *pCount = FifoCnt;
    return AD5940ERR_OK;
  }
  // AD5940_INTCClrFlag(AFEINTSRC_ALLINT);
