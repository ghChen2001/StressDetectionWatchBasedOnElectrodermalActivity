...
/* Max clock frequency for icna3311 spi is 50 MHz */
...
/**
 * @brief icna3311_dbi_async_callback_enable
 *
 * @return
 */
...
/**
 * @brief icna3311_dbi_async_callback_register
 *
 * @return
 */
...
/**
 * @brief icna3311_dbi_draw_is_busy, After the call icna3311_dbi_draw_picture_dma must check this,
 *         if icna3311_dbi_draw_is_busy() == 1, Don't allow other draw !!
...
int icna3311_dbi_init()
{
    ...
    for (uint16_t i = 0; i < (sizeof(icna3311_dbi_init_cmds) / sizeof(icna3311_dbi_init_cmd_t)); i++) {
        if (icna3311_dbi_init_cmds[i].cmd == 0xFF && icna3311_dbi_init_cmds[i].data == NULL && icna3311_dbi_init_cmds[i].databytes) {
            bflb_mtimer_delay_ms(icna3311_dbi_init_cmds[i].databytes);
        } else {
            lcd_dbi_transmit_cmd_para(icna3311_dbi_init_cmds[i].cmd, (void *)(icna3311_dbi_init_cmds[i].data), icna3311_dbi_init_cmds[i].databytes);
        }
    }

    printf("ICNA3311 init\n");
    return 0;
}
...
/**
 * @brief icna3311_dbi_set_draw_window
 *
 * @param x1
 * @param y1
 * @param x2
...
{
#if ICNA3311_DBI_OFFSET_X
    x1 += ICNA3311_DBI_OFFSET_X;
    x2 += ICNA3311_DBI_OFFSET_X;
#endif
#if ICNA3311_DBI_OFFSET_Y
    y1 += ICNA3311_DBI_OFFSET_Y;
    y2 += ICNA3311_DBI_OFFSET_Y;
#endif
...
{
    /* set window */
    icna3311_dbi_set_draw_window(x, y, x, y);

    lcd_dbi_transmit_cmd_pixel_sync(0x2C, (void *)&color, 1); // Memory Write
}

/**
 * @brief icna3311_dbi_draw_area
 *
 * @param x1
 * @param y1
 * @param x2
...
#if ICNA3311_DBI_COLOR_DEPTH == 16
    color_src = color << 16 | color;
#elif ICNA3311_DBI_COLOR_DEPTH == 32
    color_src = color;
#endif

    /* set window */
    icna3311_dbi_set_draw_window(x1, y1, x2, y2);

    lcd_dbi_transmit_cmd_pixel_fill_sync(0x2C, color_src, pixel_num); // Memory Write
}

/**
 * @brief icna3311_dbi_draw_picture_dma, Non-blocking! Using DMA acceleration, Not waiting for the draw end
 *  After the call, No other operations are allowed until (icna3311_dbi_draw_is_busy()==0)
 *
 * @param x1
 * @param y1
 * @param x2
...
/**
 * @brief icna3311_dbi_draw_picture,Blocking，Using DMA acceleration,Waiting for the draw end
 *
 * @param x1
 * @param y1
 * @param x2
